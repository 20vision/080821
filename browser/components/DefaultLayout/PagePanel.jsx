...
export default function PagePanel({page, missions}) {
  ...
  const getComponents = async() => {
    if(!componentsQueryLimitReached){
      try{
        let components_data = (await axios.get(`http://localhost:4000/get/page/${router.query.page}/components/${components.length}${router.query.mission?'/'+router.query.mission:null}`)).data
